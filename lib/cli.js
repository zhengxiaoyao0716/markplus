...
var transform = function transform(code, name) {
    return (_commander2.default.transform ? babel.transform(code, _extends({}, babelrc, { plugins: ['transform-es2015-modules-umd'], filename: name })).code : code).replace(/\r?\n/g, '\n    ').replace(/ {4}\n/g, '\n').trim();
};
...
[['--only <head|code|dump>', 'Only output the specific part.'], ['-o, --out <file>', 'Write the output into the file.'], ['-c, --config <.markplusrc|json|config.js>', 'Read the config from the file.'], ['--no-transform', 'With out babel transform.']].forEach(function (_ref4) {
