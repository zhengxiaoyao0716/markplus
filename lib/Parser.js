...
var Element = exports.Element = function () {
    ...
    _createClass(Element, [{
        ...
        key: 'dump',
        value: function dump() {
            return 'Markplus.register(' + this.at + ', ' + (this.expr || JSON.stringify(_extends({}, this.json))) + ');';
        }
    }, {
        key: 'json',
        get: function get() {
    ...
    _createClass(Save, [{
        ...
        value: function dump() {
            ...
            var lines = this.size == 1 ? '' : '    ' + ['let result;'].concat(_toConsumableArray(this.lines.slice(1).map(function (ele) {
                return ele instanceof Invalid ? '() => ' + ele.expr() + ';' : ele.params;
            }).map(function (_ref) {
                var args = _ref.args,
...
var Code = exports.Code = function (_Element7) {
    ...
    function Code(line, at, symbol, language) {
        ...
        _this7.symbol = symbol === undefined ? line : symbol;
    ...
    _createClass(Code, [{
        ...
        value: function completed(line, at) {
            ...
            return _get(Code.prototype.__proto__ || Object.getPrototypeOf(Code.prototype), 'completed', this).call(this, line, at);
        }
    }, {
        key: 'code',
        get: function get() {
            return (this.symbol ? this.lines.slice(1, this.size - 1) : this.lines.map(function (line) {
                return line.slice(4);
            })).join('\n');
        }
    }, {
        key: 'json',
        get: function get() {
            return { tag: 'pre', html: this.code, class: 'Code Code-' + this.language + ' raw-text', 'data-markplus-code-language': this.language, code: { raw: this.code } };
        }
    }]);
...
var Block = exports.Block = function (_Element9) {
    ...
    function Block(line, at, indent) {
        ...
        var _this9 = _possibleConstructorReturn(this, (Block.__proto__ || Object.getPrototypeOf(Block)).call(this, -1, Block.pipeNested(line.slice(1 + indent)), at, {}));
    ...
    _createClass(Block, [{
        ...
        value: function push(line, at) {
            ...
            var indent = mathced ? mathced[1].length : this.indents[this.indents.length - 1];
            var content = mathced ? line.slice(1 + mathced[1].length) : line;
        ...
        get: function get() {
            ...
            var folded = fold(this.lines.map(function (line, index) {
                ...
                var content = '<span data-markplus-nested="' + nested.length + '"></span>';
            ...
            var map = function map(folded) {
                return folded.map(function (line) {
                    return line instanceof Array ? '<span class="Block Block-nested">' + map(line) + '</span>' : '' + line;
                }).join('');
            };
...
Block.pipeNested = function (line, at) {
    var ele = Parser.pipe(line);
...
var More = exports.More = function (_Element10) {
    ...
    function More(line, at, symbol, brief) {
        ...
        var _this11 = _possibleConstructorReturn(this, (More.__proto__ || Object.getPrototypeOf(More)).call(this, -1, More.pipeNested(brief || '', at), at, { brief: brief }));

        _this11.symbol = symbol;
        return _this11;
    }
    ...
    _createClass(More, [{
        ...
        key: 'expr',
        get: function get() {
            var nested = [this.lines[0].expr || JSON.stringify(this.lines[0].json)];
            var html = '<span class="breif"><span id="fold">^</span><span data-markplus-nested="0"></span></span><span class="detail">' + this.lines.slice(1).map(function (line) {
                nested.push(line.expr || JSON.stringify(line.json));
...
More.pipeNested = function (line, at) {
    var ele = Parser.pipe(line);
    if (ele instanceof Save) {
        return new Plain(line, at);
    }
...
var Table = exports.Table = function (_Element11) {
    ...
    _createClass(Table, [{
        ...
        get: function get() {
            ...
            return '(() => { ' + Save.save(id, '{}') + ' return { ...' + JSON.stringify(_extends({}, json)) + ', parser: [' + parser + '], macro: [' + macro + '], table: ' + id + ' }; })()';
        }
    }]);
...
var _initialiseProps = function _initialiseProps() {
    ...
    this.pushHead = function (splited) {
        _this16.head = splited.map(function (t) {
            return t.slice(1);
        }).map(function (t) {
    ...
    this.pushAlign = function (splited) {
        ...
        if (matched.some(function (m) {
            ...
            _this16.align = [];
            _this16.pushBody(splited);
            return;
        }
        _this16.align = matched.map(function (m) {
            return m[1] + ',' + m[2];
        }).map(function (se) {
    ...
    this.pushBody = function (splited) {
        var row = splited;
        _this16.body.push(row);
    };
};

var List = exports.List = function (_Element12) {
    _inherits(List, _Element12);

    function List(line, at, symbol, content) {
        _classCallCheck(this, List);

        var _this14 = _possibleConstructorReturn(this, (List.__proto__ || Object.getPrototypeOf(List)).call(this, -1, line, at));

        _this14.symbol = symbol;
    ...
    _createClass(List, [{
        ...
        value: function completed(line, at) {
            ...
            return _get(List.prototype.__proto__ || Object.getPrototypeOf(List.prototype), 'completed', this).call(this, line, at);
        }
    }, {
        key: 'json',
        get: function get() {
            var list = [];
            var tags = this.content.startsWith('*') ? List.split(this.content.slice(1)) : (list.push([1, this.content]), new Array(10).fill().map(function (_, i) {
                return String(i);
            }));
            var carry = tags.length;
            var toIndex = carry == 1 ? function (tag) {
                return tag.match(/((?:[\0-\t\x0B\f\x0E-\u2027\u202A-\uD7FF\uE000-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF]))/g).length;
            } : function (tag) {
                return tag.match(/((?:[\0-\t\x0B\f\x0E-\u2027\u202A-\uD7FF\uE000-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF]))/g).map(function (c) {
                    return tags.indexOf(c);
                }).reverse().reduce(function (r, c, i) {
                    return r !== false && c !== -1 && r + c * Math.pow(carry, i);
                }, 0);
            };
            var toTaget = carry == 1 ? function (index) {
                return tags[0].repeat(1 + index);
            } : function (index) {
                return index === null ? '' : toTaget(parseInt(index / carry) || null) + tags[index % carry];
            };
            this.lines.slice(1).map(function (line) {
                return line.match(/\*((?:[\0-\x08\x0E-\x1F!-\x9F\xA1-\u167F\u1681-\u1FFF\u200B-\u2027\u202A-\u202E\u2030-\u205E\u2060-\u2FFF\u3001-\uD7FF\uE000-\uFEFE\uFF00-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF])*)[\t-\r \xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\uFEFF]((?:[\0-\t\x0B\f\x0E-\u2027\u202A-\uD7FF\uE000-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF])*)/).slice(1, 3);
            }).forEach(function (_ref14) {
                var _ref15 = _slicedToArray(_ref14, 2),
                    tag = _ref15[0],
                    text = _ref15[1];

                return list.push([tag ? toIndex(tag) : list[list.length - 1] ? 1 + list[list.length - 1][0] : 0, text]);
            });
            return {
                tag: 'span',
                html: list.map(function (_ref16) {
                    var _ref17 = _slicedToArray(_ref16, 2),
                        tag = _ref17[0],
                        text = _ref17[1];

                    return '<span class="item"><span class="tag">' + toTaget(tag) + '</span><span>' + text + '</span></span>';
                }).join(''),
...
List.symbol = '*';
List.split = withEscape(function (text) {
    var splited = text.split('*').map(function (sep) {
        return sep.trim();
    });
    return function (reg, rep) {
        return splited ? splited.map(function (text) {
            return text.replace(reg, rep);
        }).reduce(function (seps, sep) {
            return [].concat(_toConsumableArray(seps), _toConsumableArray(function (matched) {
                return matched.length == 2 ? [].concat(_toConsumableArray(new Array(matched[1] - matched[0] + 1).fill().map(function (_, i) {
                    return matched[0] + i;
                }))) : matched;
            }(sep.match(/((?:[\0-\t\x0B\f\x0E-\u2027\u202A-\uD7FF\uE000-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF]))/g).map(function (c) {
...
var Parser = {
    ...
    HTMLElement: {
        ...
        pipe: function pipe(line, at) {
            ...
            }, Array.from(List.symbol).reduce(function (o, s) {
                return o[s] = function () {
                    return new List(line, at, s, content);
                }, o;
            }, {}), {
                '': function _() {
                    return line.startsWith('    ') ? new Code(line, at, '', 'text') : new Plain(line, at);
    ...
    parse: function parse(content) {
        ...
        lines.forEach(function (line, index) {
            ...
            if (ele.completed(line, at)) {
                elements.unshift(_this15.pipe(line, at));
                return;
            }
            ele.push(line, at);
        });
